...
# time python -i /Users/silver/Dropbox/bin/phd_87a_uplim_find_coords_vlt.py
...
#########
# Parameters
DAT_DIR = "/Users/silver/Dropbox/phd/data/vlt/87a/4ali/"
WRK_DIR = "/Users/silver/Dropbox/phd/projects/87a/uplim/"
RINGEST = "/Users/silver/Dropbox/phd/data/vlt/87a/4ali/nac_k_2012-00-00_red_al2.fits"
...
files = glob(DAT_DIR+'/nac*.fits') #Find all files.
NN = len(files)
coords = []
boxes = np.array([554,619,549,634])
...
# Plots
    ...
    fig.savefig('coord_fits/vlt/' + img_path[-29:-5] + '_ell_img.pdf',bbox_inches='tight', pad_inches=0.01)
    plt.close(fig)
    
    fig = plt.figure()
    plt.imshow(img-last_ell, cmap='afmhot', origin='lower', interpolation=INTERPOLATION)
    fig.savefig('coord_fits/vlt/' + img_path[-29:-5] + '_ell_res.pdf',bbox_inches='tight', pad_inches=0.01)
    plt.close(fig)
...
# Plots
    ...
    fig.savefig('coord_fits/vlt/' + img_path[-29:-5] + '_gau_img.pdf',bbox_inches='tight', pad_inches=0.01)
    plt.close(fig)
 
    fig = plt.figure()
    plt.imshow(img-last_gau, cmap='afmhot', origin='lower', interpolation=INTERPOLATION)
    fig.savefig('coord_fits/vlt/' + img_path[-29:-5] + '_gau_res.pdf',bbox_inches='tight', pad_inches=0.01)
    plt.close(fig)
    
#########
# Plot and wrap-up
img = fits.open(RINGEST)[0].data[boxes[0]:boxes[1],boxes[2]:boxes[3]]
coords.append([592.158345778, 586.775275464])
coords = np.array(coords)

fig = plt.figure()
col = ['or','og','oc','om','ow','ok','ok']

for i in range(0,coords.shape[0]):
    plt.plot(coords[i,0]-boxes[2], coords[i,1]-boxes[0],col[i])

plt.imshow(img, cmap='afmhot', origin='lower', interpolation=INTERPOLATION, norm=LogNorm(vmin=np.amax(img)/100, vmax=np.amax(img)))
fig.savefig('coord_fits/vlt/' + RINGEST[-29:-5] + '_coords.pdf',bbox_inches='tight', pad_inches=0.01)
plt.show()
plt.close(fig)

print np.mean(coords[0:3,0]), np.mean(coords[0:3,1])
